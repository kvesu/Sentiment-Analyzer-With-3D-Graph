...
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"   # helps some macOS OpenMP builds
os.environ["XGB_NUM_THREADS"] = "1"           # for some xgboost builds

# --- multiprocessing safety on macOS ---
import multiprocessing as mp
try:
    mp.set_start_method("spawn", force=True)
except RuntimeError:
    # it's okay if it's already set
    pass

def lazy_nlp_imports():
    # Delay heavy imports until the env is configured
    global SimplifiedPriceAnalyzer, NewsProcessor, EnhancedNewsProcessor
...
from db import upsert_article, get_or_create_ticker, upsert_article_ticker, insert_prediction
from sentiment_scorer import SentimentScorer



def _force_xgb_single_thread(model):
    """Try to force XGBoost/Sklearn wrappers to run single-threaded at predict time."""
    try:
        # XGBoost sklearn API (XGBRegressor/XGBClassifier)
        if hasattr(model, "get_booster"):
            booster = model.get_booster()
            if booster is not None:
                booster.set_param({"nthread": 1})
        # Some builds accept set_params
...
class ContinuousPredictionEngine:
    def __init__(self, target_horizon):
        ...
        # NEW: sentiment scorer (VADER)
        self.sentiment_scorer = SentimentScorer(
            use_vader=True,
            use_finbert=False,
        )
        # NEW: track per-ticker daily sentiment (simple running averages)
        from datetime import date
        self.daily_sentiment_date = datetime.now(MARKET_TIMEZONE).date()
        # ticker -> dict with running sums and counts
        self.daily_sentiment = {}  # e.g., {'AAPL': {'sum_vader': ..., 'count_vader': ..., 'sum_combined': ..., 'count_combined': ...}}
    ...
    def load_components(self):
        try:
            ...
            _force_xgb_single_thread(self.model)
            _force_xgb_single_thread(self.gk_model)
    ...
    def _reset_daily_sentiment_if_new_day(self, news_dt):
        """Reset daily sentiment stats if the news datetime is on a new day
        compared to the currently tracked day."""
        news_date = news_dt.astimezone(MARKET_TIMEZONE).date()
        if news_date != self.daily_sentiment_date:
            logger.info(
                f"New trading day detected (old: {self.daily_sentiment_date}, new: {news_date}). "
                f"Resetting daily sentiment stats."
            )
            self.daily_sentiment_date = news_date
            self.daily_sentiment = {}

    def _update_daily_sentiment(self, ticker, sent_vader, sent_combined, news_dt):
        """
        Update running sums / counts for a ticker's daily sentiment.
        """
        # Ensure we're on the right day
        self._reset_daily_sentiment_if_new_day(news_dt)

        if ticker not in self.daily_sentiment:
            self.daily_sentiment[ticker] = {
                "sum_vader": 0.0,
                "count_vader": 0,
                "sum_combined": 0.0,
                "count_combined": 0,
        ...
        stats = self.daily_sentiment[ticker]

        if sent_vader is not None:
            stats["sum_vader"] += float(sent_vader)
            stats["count_vader"] += 1

        if sent_combined is not None:
            stats["sum_combined"] += float(sent_combined)
            stats["count_combined"] += 1

    def _get_daily_sentiment(self, ticker):
        """
        Return (avg_vader, avg_combined) for the ticker for the current day,
        or (None, None) if no data.
        """
        stats = self.daily_sentiment.get(ticker)
        if not stats:
            return None, None

        avg_vader = None
        if stats["count_vader"] > 0:
            avg_vader = stats["sum_vader"] / stats["count_vader"]

        avg_combined = None
        if stats["count_combined"] > 0:
            avg_combined = stats["sum_combined"] / stats["count_combined"]

        return avg_vader, avg_combined
    ...
    def generate_predictions(self, articles_for_processing):
        ...
        try:
            ...
            for row, pred in zip(filtered_df.to_dict("records"), preds):
                # --- Build text for VADER (headline + article text if available) ---
                text_parts = []
                if row.get("headline"):
                    text_parts.append(str(row["headline"]))
                if row.get("text"):  # full article html/text if present
                ...
                if text:
                    try:
                        s = self.sentiment_scorer.score(text)
                        sent_vader = s.vader
                    except Exception as e:
                        logger.error(f"VADER sentiment scoring failed: {e}")

                # existing combined sentiment from EnhancedNewsProcessor
                sentiment_combined = row.get("sentiment_combined", 0.0)

                # --- NEW: update daily sentiment stats for this ticker ---
                self._update_daily_sentiment(
                    ticker=row["ticker"],
                    sent_vader=sent_vader,
                    sent_combined=sentiment_combined,
                    news_dt=row["datetime"],
                )

                # --- NEW: get current daily averages for this ticker ---
                daily_vader, daily_combined = self._get_daily_sentiment(row["ticker"])

                # --- Build prediction dict with sentiment weights + VADER + daily aggregates ---
                pred_record = {
                    ...
                    # your current sentiment weights from EnhancedNewsProcessor
                    "sentiment_dynamic": round(float(row.get("sentiment_dynamic", 0)), 4),
                    "sentiment_ml": round(float(row.get("sentiment_ml", 0)), 4),
                    "sentiment_keyword": round(float(row.get("sentiment_keyword", 0)), 4),
                    "sentiment_combined": round(float(sentiment_combined), 4),

                    # per-article VADER
                    "sent_vader": sent_vader,

                    # NEW: daily aggregates (up to this news item)
                    "daily_sentiment_vader": daily_vader,
                    "daily_sentiment_combined": daily_combined,

                    # NEW: STD channel features (from TA)
                    "std_dist_upper": row.get("std_dist_upper"),
                    "std_dist_lower": row.get("std_dist_lower"),
                    "std_channel_width": row.get("std_channel_width"),
                ...
                # --- Save to MySQL ---
            try:
                logger.info("Attempting to save %d predictions to MySQL", len(predictions))

                for row, p in zip(filtered_df.to_dict("records"), predictions):
                    # 1) Upsert the article
                    url = row.get("url") or row.get("link") or ""
                    published = row.get("datetime")  # NOTE: don't call this 'datetime'

                    if hasattr(published, "astimezone"):
                        published_utc = published.astimezone(timezone.utc).replace(tzinfo=None)
                    else:
                        published_utc = published

                    article_id = upsert_article(
                        url=url,
                        headline=row.get("headline", "")[:65535],
                        source=row.get("source"),
                        published_dt_utc=published_utc,
                        scraped_html=None,
                        full_text=None,
                    )

                    # 2) Ensure ticker exists
                    ticker_id = get_or_create_ticker(row.get("ticker", ""))

                    # 3) Upsert article_ticker features
                    at_fields = {
                        "headline_sentiment": float(row.get("sentiment_combined", 0.0)),
                        "market_session": row.get("market_session", "UNKNOWN"),
                        "news_age_minutes": float(row.get("news_age_minutes", 0.0)),
                    }
                    at_fields = {k: v for k, v in at_fields.items() if v is not None}

                    article_ticker_id = upsert_article_ticker(
                    ...
                    predicted_ret = float(p[f"predicted_{self.target_column}"])

                    insert_prediction(
                        article_ticker_id=article_ticker_id,
                        horizon=p["horizon"],
                        gk_prob=float(p["prediction_confidence"]),
                        predicted_pct=predicted_ret,
                        prediction_time_utc=datetime.now(timezone.utc).replace(tzinfo=None),
                    )

                logger.info("Saved %d predictions to MySQL", len(predictions))

            except Exception as db_err:
                logger.error(f"Failed to insert predictions into MySQL: {db_err}")
            ...
            try:
                ...
                if should_process:
                    ...
                    if predictions:
                        ...
                        # === ensure only one entry per ticker in Top 5 display ===
                        unique_ticker_preds = {}
