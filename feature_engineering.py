...
class FinancialNewsFeatureEngineer:
    ...
    def calculate_ta_features_for_one_row(self, past_data: pd.DataFrame) -> dict:
        ...
        features = {
            ...
            # NEW: STD channel features
            'std_mid': np.nan,
            'std_upper': np.nan,
            'std_lower': np.nan,
            'std_dist_upper': np.nan,
            'std_dist_lower': np.nan,
            'std_channel_width': np.nan,
        }
            ...
            if len(past_data_copy) >= 200:
                try:
                    ...
                    print(f"Error calculating SMA200: {e}")
                        ...
                        # --- NEW: STD channel features (like Bollinger-style bands) ---
            try:
                window = 20
                k = 2.0

                if len(past_data_copy) >= window:
                    roll = past_data_copy[price_col].rolling(window=window, min_periods=window // 2)
                    roll_mean = roll.mean()
                    roll_std = roll.std()

                    mid = roll_mean.iloc[-1]
                    std = roll_std.iloc[-1]

                    if not pd.isna(mid) and not pd.isna(std) and current_price != 0:
                        upper = mid + k * std
                        lower = mid - k * std

                        features['std_mid'] = mid
                        features['std_upper'] = upper
                        features['std_lower'] = lower

                        # Relative distances: normalized to current price
                        features['std_dist_upper'] = (current_price - upper) / current_price
                        features['std_dist_lower'] = (current_price - lower) / current_price
                        features['std_channel_width'] = (upper - lower) / current_price
            except Exception as e:
                print(f"Error calculating STD channel features: {e}")
